/*!
 * @file AirgradientIICSerial.h
 * @brief Define the basic structure of class AirgradientIICSerial
 *
...
void AirgradientIICSerial::end() { subSerialGlobalRegEnable(_subSerialChannel, rst); }

int AirgradientIICSerial::available(void) {
  int index;
  uint8_t val = 0;
  sFsrReg_t fsr;
...
// size_t AirgradientIICSerial::write(uint8_t value) {
//   sFsrReg_t fsr;
//   fsr = readFIFOStateReg();
//   if (fsr.tFull == 1) {
...
size_t AirgradientIICSerial::read(void *pBuf, size_t size) {
  if (pBuf == NULL) {
    ESP_LOGV(TAG, "pBuf ERROR!! : null pointer");
    return 0;
  }
...
void AirgradientIICSerial::flush(void) {
  sFsrReg_t fsr = readFIFOStateReg();
  while (fsr.tDat == 1)
    ;
}

void AirgradientIICSerial::subSerialConfig(uint8_t subUartChannel) {
...
void AirgradientIICSerial::subSerialGlobalRegEnable(uint8_t subUartChannel, eGlobalRegType_t type) {
...
void AirgradientIICSerial::subSerialPageSwitch(ePageNumber_t page) {
  if (page >= pageTotal) {
    return;
  }
  uint8_t val = 0;
...
void AirgradientIICSerial::subSerialRegConfig(uint8_t reg, void *pValue) {
  uint8_t val = 0;
  readReg(reg, &val, 1);
  ESP_LOGV(TAG, "before: %.2x", val);
  val |= *(uint8_t *)pValue;
...
uint8_t AirgradientIICSerial::getGlobalRegType(eGlobalRegType_t type) {
...
void AirgradientIICSerial::setSubSerialBaudRate(unsigned long baud) {
...
void AirgradientIICSerial::setSubSerialConfigReg(uint8_t format, eCommunicationMode_t mode,
                                                 eLineBreakOutput_t opt) {
...
uint8_t AirgradientIICSerial::updateAddr(uint8_t pre, uint8_t subUartChannel, uint8_t obj) {
  sIICAddr_t addr = {.type = obj, .uart = subUartChannel, .addrPre = (uint8_t)((int)pre >> 3)};
  return *(uint8_t *)&addr;
}

AirgradientIICSerial::sFsrReg_t AirgradientIICSerial::readFIFOStateReg() {
  sFsrReg_t fsr;
  readReg(REG_WK2132_FSR, &fsr, sizeof(fsr));
  return fsr;
}

uint8_t AirgradientIICSerial::subSerialChnnlSwitch(uint8_t subUartChannel) {
  uint8_t channel = _subSerialChannel;
  _subSerialChannel = subUartChannel;
  return channel;
}

void AirgradientIICSerial::turnOffClock() {
  // uint8_t valGENA = 0;
  // // GENA
...
void AirgradientIICSerial::printAllRegsForCurrentCh() {
...
bool AirgradientIICSerial::isChannelInSleep(uint8_t subUartChannel) {
  uint8_t val = 0;
...
void AirgradientIICSerial::sleep() {}

void AirgradientIICSerial::wakeup() {}

void AirgradientIICSerial::writeReg(uint8_t reg, const void *pBuf, size_t size) {
  if (pBuf == NULL) {
    ESP_LOGV(TAG, "pBuf ERROR!! : null pointer");
    return;
  }
...
uint8_t AirgradientIICSerial::readFIFO(void *pBuf, size_t size) {
  if (pBuf == NULL) {
    ESP_LOGV(TAG, "pBuf ERROR!! : null pointer");
    return 0;
  }
...
void AirgradientIICSerial::writeFIFO(void *pBuf, size_t size) {
  if (pBuf == NULL) {
    ESP_LOGV(TAG, "pBuf ERROR!! : null pointer");
    return;
  }
...
i2c_master_dev_handle_t AirgradientIICSerial::getDeviceHandle(uint8_t addr) {
