...
public:
   ...
   */
  bool setAlertPinMode(ADS1115_COMP_QUE mode);
   ...
   */
  bool setAlertLatch(ADS1115_LATCH latch);
   ...
   */
  bool setAlertPol(ADS1115_ALERT_POL polarity);
   ...
   * ADS1115_WINDOW
   */
  bool setAlertModeAndLimit_V(ADS1115_COMP_MODE mode, float hithres, float lothres);
   ...
   * ADS1115_860_SPS
   */
  bool setConvRate(ADS1115_CONV_RATE rate);
   ...
   */
  bool setMeasureMode(ADS1115_MEASURE_MODE mode);
   ...
   */
  bool setVoltageRange_mV(ADS1115_RANGE range);
   ...
   */
  bool setCompareChannels(ADS1115_MUX mux);

  /* Set to channel (0-3) in single ended mode in a non blocking way without delay
   */
  bool setCompareChannels_nonblock(ADS1115_MUX mux);

  /* Set to channel (0-3) in single ended mode
   */
  bool setSingleChannel(size_t channel);
...
protected:
  ...
  esp_err_t writeRegister(uint8_t reg, uint16_t val);
  esp_err_t readRegister(uint8_t reg, uint16_t *output);
  uint16_t readRegisterDefault(uint8_t reg);
  int map(int x, int in_min, int in_max, int out_min, int out_max); // arduino map function
};
